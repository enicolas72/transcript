...
accounts, don't collect analytics, don't sell anything inside the app,
and don't share your data with anyone else.

xTranscript is free and open source (MIT). You can read exactly what
the app does at https://github.com/enicolas72/transcript.

## What data xTranscript handles

- **Audio files you pick.** When you drop an audio or video file onto
  xTranscript, the app decodes its audio track on your machine with an
  embedded LGPL build of FFmpeg. The decoded audio is streamed (as
  16 kHz mono PCM) over a secure WebSocket to `wss://api.x.ai/v1/stt`,
  and the transcript is returned. FFmpeg itself runs entirely on your
  device and does not initiate any network connections of its own (we
  built it with all network protocols disabled).
...
- No in-app purchases, no subscriptions, no payment data of any kind.
...
- Input audio/video files are read from disk and decoded in memory in
  small chunks. xTranscript never copies them and writes no temporary
  files.
